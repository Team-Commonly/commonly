...
describe('V2 routing', () => {
  ...
  test('AI agent instructions guide renders its role contract after the app takes over', async () => {
    renderAt('/guides/how-to-write-ai-agent-instructions/');
    expect(await screen.findByRole('heading', { level: 1, name: 'How to Write AI Agent Instructions: A Practical Role Contract' })).toBeInTheDocument();
    expect(screen.getAllByText(/role contract/).length).toBeGreaterThan(0);
  });
  ...
  test('AI agents for customer support guide renders its triage packet after the app takes over', async () => {
    renderAt('/guides/ai-agents-for-customer-support/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agents for Customer Support: Triage, Context, and Safe Handoffs' })).toBeInTheDocument();
    expect(screen.getAllByText(/triage packet/).length).toBeGreaterThan(0);
  });

  test('multi-agent system guide renders its operating model after the app takes over', async () => {
    renderAt('/guides/what-is-a-multi-agent-system/');
    expect(await screen.findByRole('heading', { level: 1, name: 'What Is a Multi-Agent System? A Practical Definition for Teams' })).toBeInTheDocument();
    expect(screen.getAllByText(/operating model/).length).toBeGreaterThan(0);
  });

  test('AI agent glossary renders its shared vocabulary after the app takes over', async () => {
    renderAt('/guides/ai-agent-glossary/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agent Glossary: Essential Terms for Building Agent Teams' })).toBeInTheDocument();
    expect(screen.getAllByText(/shared vocabulary/).length).toBeGreaterThan(0);
  });

  test('AI agent governance guide renders its review trigger after the app takes over', async () => {
    renderAt('/guides/ai-agent-governance/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agent Governance: Roles, Permissions, and Accountable Review' })).toBeInTheDocument();
    expect(screen.getAllByText(/review trigger/).length).toBeGreaterThan(0);
  });

  test('AI software-development guide renders its change contract after the app takes over', async () => {
    renderAt('/guides/ai-agents-for-software-development/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agents for Software Development: Prepare Changes Without Owning the Merge' })).toBeInTheDocument();
    expect(screen.getAllByText(/change contract/).length).toBeGreaterThan(0);
  });

  test('AI agent roles guide renders its stop condition after the app takes over', async () => {
    renderAt('/guides/ai-agent-roles/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agent Roles: Define Work, Boundaries, and Handoffs' })).toBeInTheDocument();
  ...
  test('AI agent acceptance-criteria guide renders its safe stop after the app takes over', async () => {
    renderAt('/guides/ai-agent-acceptance-criteria/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agent Acceptance Criteria: Define Work Agents Can Be Evaluated Against' })).toBeInTheDocument();
  ...
  test('AI agent no-op guide renders its eligibility rule after the app takes over', async () => {
    renderAt('/guides/ai-agent-no-op/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agent No-Op: When Doing Nothing Is the Right Result' })).toBeInTheDocument();
  ...
  test('AI agent work-contract guide renders its permitted operations after the app takes over', async () => {
    renderAt('/guides/ai-agent-work-contract/');
    expect(await screen.findByRole('heading', { level: 1, name: 'AI Agent Work Contract: Define One Bounded, Reviewable Task' })).toBeInTheDocument();
