...
const AgentsHub = ({ currentPodId: propPodId = null }) => {
  ...
  const applyConfigDialogState = (resolved) => {
    ...
    setConfigSelectedScopes(normalizedCurrentScopes.filter((scope) => knownScopeIds.has(scope as string)));
    setConfigExtraScopes(normalizedCurrentScopes.filter((scope) => !knownScopeIds.has(scope as string)));
  ...
  useEffect(() => {
    ...
    const fetchGateways = async () => {
      ...
      try {
        const response = await axios.get('/api/gateways', { headers: getAuthHeaders() });
  ...
  const fetchRuntimeStatus = async () => {
    ...
    try {
      const instanceId = configAgent.instanceId || 'default';
      const gatewayId = resolveRuntimeGatewayId();
      const params: Record<string, any> = { instanceId };
      if (gatewayId) {
  ...
  const handleStartRuntime = async () => {
    ...
    try {
      const instanceId = configAgent.instanceId || 'default';
      const gatewayId = resolveRuntimeGatewayId();
      const payload: Record<string, any> = { instanceId };
      if (gatewayId) {
        payload.gatewayId = gatewayId;
      }
      await axios.post(
  ...
  const handleClearRuntimeSessions = async () => {
    ...
    try {
      const instanceId = configAgent.instanceId || 'default';
      const gatewayId = resolveRuntimeGatewayId();
      const payload: Record<string, any> = { instanceId, restart: true };
      if (gatewayId) {
        payload.gatewayId = gatewayId;
      }
      const response = await axios.post(
  ...
  const fetchRuntimeLogs = async () => {
    ...
    try {
      ...
      const instanceId = configAgent.instanceId || 'default';
      const gatewayId = resolveRuntimeGatewayId();
      const params: Record<string, any> = { lines: runtimeLogsLines, instanceId };
      if (gatewayId) {
        params.gatewayId = gatewayId;
      }
      const response = await axios.get(
              ...
              {loading
                ? [1, 2, 3, 4, 5, 6].map((i) => (
                    <Grid item xs={12} sm={6} md={4} lg={4} key={i}>
                      <AgentCard loading agent={null as any} />
                    </Grid>
                  ))
                ...
                <Grid item xs={12} sm={6} md={4} lg={4} key={`${agent.name}-${agent.instanceId || 'default'}`}>
                  <AgentCard
                    agent={{
                      ...
                      presetId: (agent as any).config?.presetId || null,
                      customizations: (agent as any).config?.customizations || null,
                    } as any}
                    installed
              ...
              >
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }}>
                  <TextField
                    ...
                    onChange={(e) => setAdminAutonomyHours(Number(e.target.value))}
                  ...
                  <TextField
                    ...
                    onChange={(e) => setAdminAutonomyMinMatches(Number(e.target.value))}
              ...
              {!skillSyncAllPods && (
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <Select
                    ...
                    multiple
                    value={skillSyncPods}
                    onChange={(event) => setSkillSyncPods(event.target.value as any[])}
              ...
              {skillSyncMode === 'selected' && (
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <Select
                    ...
                    onChange={(event) => setSkillSyncSkills(event.target.value as any[])}
