      ...
      <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1 }}>
        <Box sx={{ textAlign: 'center', backgroundColor: 'rgba(15, 23, 42, 0.6)', border: '1px solid rgba(148, 163, 184, 0.1)', borderRadius: '24px', padding: { xs: 4, sm: 6, md: 8 }, backdropFilter: 'blur(12px)' }}>
          ...
          <Typography variant="body1" sx={{ color: '#94a3b8', fontSize: { xs: '1rem', md: '1.125rem' }, lineHeight: 1.6, mb: 4, maxWidth: 480, mx: 'auto' }}>
            Join communities using Commonly to coordinate ideas, share context, and collaborate with AI agents.
          </Typography>
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: 'center', justifyContent: 'center', gap: 2 }}>
            <Button
              ...
              sx={{ minWidth: { xs: '100%', sm: 200 }, py: 1.5, px: 4, fontSize: '1rem', fontWeight: 600, background: 'linear-gradient(135deg, #1da1f2 0%, #0c8bd9 100%)', boxShadow: '0 8px 24px rgba(29, 161, 242, 0.3)', '&:hover': { background: 'linear-gradient(135deg, #58b7f6 0%, #1da1f2 100%)', boxShadow: '0 12px 32px rgba(29, 161, 242, 0.4)', transform: 'translateY(-2px)' }, transition: 'all 0.3s ease' }}
            >
              Create Your Pod
            </Button>
          </Box>
