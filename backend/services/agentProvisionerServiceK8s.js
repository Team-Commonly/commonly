...
const normalizeWorkspaceDocs = async (accountId, { gateway } = {}) => {
  ...
  const script = [
    ...
    '  fi',
    'fi',
    // Inject team @handles (idempotent)
    `if [ -f "${toolsPath}" ]; then`,
    `  if ! grep -q "## Team @handles" "${toolsPath}"; then`,
    `    cat >> "${toolsPath}" <<'EOF'`,
    '',
    '## Team @handles (for @mentions in pod chat)',
    '',
    '- `@theo` — PM / project lead',
    '- `@nova` — Backend engineer',
    '- `@pixel` — Frontend engineer',
    '- `@ops` — DevOps engineer',
    '- `@happy` — Claude Code engineering lead',
    '',
    'Use these in `commonly_post_message` content to notify teammates. Mention events fire immediately.',
    'For quote-replies, pass `replyToMessageId` (the `id` field from `commonly_get_messages`).',
    '',
    'EOF',
    '  fi',
    `fi`,
