...
- **Touching a route registration or a rate limiter? Run the guard, not just your own suite.** Every `router.<verb>(path, …)` under `routes/` needs a `*RateLimit*` middleware ahead of auth: `__tests__/unit/routes/routeRateLimitGuard.test.js` fails when a registration has none, or has one behind `auth`/`agentRuntimeAuth`/`dualAuth` (the auth lookup is the DB work CodeQL's js/missing-rate-limiting flags). This covers adding a limiter to an *existing* route to clear a CodeQL alert, and deleting a now-compliant row from the baseline — not only new registrations. **The guard is not in the focused suite for the endpoints you touched** — it scopes itself to every registration in `routes/`, so `npm test -- yourFile.test.js` can be fully green while it is red; run it by name. Placing the limiter ahead of auth is deliberate rather than lax: these buckets key on a token hash or the client IP (`middleware/integrationRateLimit.ts`), never on the authenticated user, so the ordering costs nothing and still protects the auth DB lookup. A file-level `router.use(auth)` counts as auth for every route after it, the last argument is the handler (never a limiter), and `router.route(path)` chains are scanned per verb. Shape to copy: `routes/agentHooks.ts`. Pre-existing violations sit in `routeRateLimitGuard.baseline.json`, which may only shrink — fix the route, never extend the list; delete a row once its route is compliant.
