...
async function registerDiscordCommands() {
  ...
  try {
    ...
    integrations.forEach((integration, index) => {
      ...
      console.log(
        `   Server ID: ${integration.config.serverId || "Missing"}\n`,
      );
    });
    ...
    if (integrations.length === 1) {
      ...
      console.log(
        '(Enter the number, or "all" to register for all integrations)',
      );
...
async function registerCommandsForIntegration(integration) {
  ...
  try {
    ...
    const success = await discordService.registerSlashCommands(
