...
class DiscordCommandDeployment {
  ...
  async registerGlobalCommands() {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        const success = await this.attemptGlobalRegistration(commands, DISCORD_CLIENT_ID, DISCORD_BOT_TOKEN);
  ...
  generateReport() {
    ...
    console.log(`🎯 Success rate: ${this.results.total > 0 ? Math.round((this.results.registered / this.results.total) * 100) : 0}%`);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (missingCommands.length === 0) {
          console.log(`✅ All ${expectedCommands.length} commands verified globally`);
          return { success: true, registeredCommands: foundCommands };
        }
        console.log(`❌ Missing commands: ${missingCommands.join(', ')}`);
        return { success: false, missingCommands, registeredCommands: foundCommands };
      }
      throw new Error(`Discord API returned status ${response.status}`);
    } catch (error) {
...
async function main() {
  ...
  switch (command) {
    ...
    case 'help':
      ...
      console.log('');
      console.log('Usage:');
      console.log('  node scripts/deploy-discord-commands.js deploy    - Deploy global commands');
      console.log('  node scripts/deploy-discord-commands.js verify    - Verify deployment status');
      console.log('  node scripts/deploy-discord-commands.js help      - Show this help');
      console.log('');
