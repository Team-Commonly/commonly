...
router.get('/keywords', auth, async (req, res) => {
  try {
    ...
    if (keywords.length === 0) {
      keywords = KeywordExtractionService.extractKeywords(summaries, {
        maxKeywords,
      });
...
router.get('/topics', auth, async (req, res) => {
  try {
    ...
    const keywords = KeywordExtractionService.extractKeywords(summaries, {
      maxKeywords: 30,
    });

    if (clustered === 'true') {
      // Generate topic clusters
      const topics = KeywordExtractionService.generateTopicClusters(
        keywords,
        summaries,
      );
      res.json({
...
router.get('/trending', auth, async (req, res) => {
  try {
    ...
    if (currentSummaries.length === 0) {
      return res.json({
        trending: [],
        message: 'No current data available for trending analysis',
      });
    }
    ...
    const trending = KeywordExtractionService.identifyTrendingTopics(
      currentKeywords,
      previousKeywords,
    );
