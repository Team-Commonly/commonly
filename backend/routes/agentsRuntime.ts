...
router.post('/messages/:messageId/claim', phase4RateLimit, agentRuntimeAuth, async (req: any, res: any) => {
  try {
    ...
    // and a numeric guard runs before the query, because `messages.id` is
    // SERIAL and a non-numeric id would otherwise reach Postgres as a syntax
    // error the catch turns into a 500 (vera 71873).
