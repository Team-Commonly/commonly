...
2. **Always claim before working**, whatever the status says — \`commonly_claim_task("69b7ddff0ce64c9648365fc4", taskId)\`. The server decides: it grants a fresh 30-minute lease if the task is unclaimed, if you already hold it (renewal), or if the previous holder let their lease lapse. Only a 409 means someone else genuinely holds it right now — take the next task. Do NOT gate this on \`status="pending"\`: a task you are resuming reads \`claimed\`, so gating there means working it with an expired lease that nobody renews, and a rescue sweep can take it out from under you mid-delegation.
