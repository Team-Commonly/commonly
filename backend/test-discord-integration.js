...
async function testWebhookConnection() {
  ...
  try {
    const response = await axios.post(`${TEST_CONFIG.baseURL}/api/discord/test-webhook`, {
...
async function testCreateIntegration() {
  ...
  try {
    const response = await axios.post(`${TEST_CONFIG.baseURL}/api/discord/integration`, {
    ...
    console.error('❌ Integration creation error:', error.response?.data || error.message);
    return null;
  }
}
...
async function testGetIntegrations() {
  ...
  try {
    const response = await axios.get(`${TEST_CONFIG.baseURL}/api/integrations/${TEST_CONFIG.podId}`, {
      headers: {
        Authorization: `Bearer ${TEST_CONFIG.authToken}`,
      },
    });
    ...
    response.data.forEach((integration, index) => {
      console.log(`  ${index + 1}. ${integration.type} - ${integration.status}`);
    });
    return response.data;
  } catch (error) {
    console.error('❌ Get integrations error:', error.response?.data || error.message);
    return [];
  }
}
...
async function testSendDiscordMessage(integrationId) {
  ...
  try {
    const _response = await axios.post(`${TEST_CONFIG.baseURL}/api/integrations/${integrationId}/send`, {
...
async function testFetchMessages(integrationId) {
  ...
  try {
    const _response = await axios.get(`${TEST_CONFIG.baseURL}/api/integrations/${integrationId}/messages`, {
      headers: {
        Authorization: `Bearer ${TEST_CONFIG.authToken}`,
      },
    });
...
async function testGetStats(integrationId) {
  ...
  try {
    const _response = await axios.get(`${TEST_CONFIG.baseURL}/api/discord/stats/${integrationId}`, {
      headers: {
        Authorization: `Bearer ${TEST_CONFIG.authToken}`,
      },
    });
...
async function testDeleteIntegration(integrationId) {
  ...
  try {
    const _response = await axios.delete(`${TEST_CONFIG.baseURL}/api/integrations/${integrationId}`, {
      headers: {
        Authorization: `Bearer ${TEST_CONFIG.authToken}`,
      },
    });
    ...
    console.error('❌ Delete integration error:', error.response?.data || error.message);
    return false;
  }
}
...
async function runTests() {
  ...
  if (TEST_CONFIG.webhookUrl === 'YOUR_WEBHOOK_URL'
      || TEST_CONFIG.botToken === 'YOUR_BOT_TOKEN'
      || TEST_CONFIG.podId === 'YOUR_POD_ID'
      || TEST_CONFIG.authToken === 'YOUR_JWT_TOKEN') {
    console.log('⚠️  Please update TEST_CONFIG with your actual values before running tests.');
