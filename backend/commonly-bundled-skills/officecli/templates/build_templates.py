...
def build_xlsx():
    ...
    NOTE (2026-05-08): the live `commonly-data.xlsx` in this directory is
    NOT generated by this function — it's built in the gateway pod with
    officecli's native commands (`officecli create` + `officecli set`)
    because openpyxl's `Font(color=…, bold=True, …)` produces a styles.xml
    that officecli's own validator rejects with "unexpected child element
    'color' in font[1]" (font children are accepted by the OOXML spec but
    officecli is stricter on ordering). Until openpyxl emits a font child
    layout officecli accepts, regenerate via:

        kubectl exec -n commonly-dev deployment/clawdbot-gateway -- bash -c '
          cd /tmp && rm -rf rebuild && mkdir rebuild && cd rebuild
          officecli create commonly-data.xlsx
          # row 1 (header)
          officecli add commonly-data.xlsx /Sheet1 --type row
          for col_val in "A:Item" "B:Status" "C:Owner" "D:Notes" "E:Updated"; do
            col=${col_val%:*}; v=${col_val#*:}
            officecli set commonly-data.xlsx /Sheet1/${col}1 --prop value="$v"
          done
          # row 2 ({{key}} placeholders)
          officecli add commonly-data.xlsx /Sheet1 --type row
          for col_key in "A:sample_item" "B:sample_status" "C:sample_owner" \
                         "D:sample_notes" "E:sample_updated"; do
            col=${col_key%:*}; k=${col_key#*:}
            officecli set commonly-data.xlsx /Sheet1/${col}2 --prop value="{{${k}}}"
          done
          officecli close commonly-data.xlsx
        '

    The function below is kept as the historical reference — useful if
    openpyxl ever fixes the font-child-order issue, OR if we move to a
    different officecli version with a more permissive validator.
