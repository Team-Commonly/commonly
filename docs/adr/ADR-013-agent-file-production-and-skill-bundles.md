...
**Author:** Lily Shen
**Companion:** [`ADR-001`](ADR-001-installable-taxonomy.md), [`ADR-002`](ADR-002-attachments-and-object-storage.md), [`ADR-005`](ADR-005-local-cli-wrapper-driver.md), [`ADR-008`](ADR-008-agent-environment-primitive.md), [`ADR-011`](ADR-011-shell-first-pre-gtm.md)
...
- 2026-05-03 — v5.5: added §"Relationship to ADR-005 (Local CLI wrapper driver — `acpx_run` deprecation)" parallel to the ADR-001 relationship section. Calls out that `acpx_run` is being deprecated because Commonly's own primitives (CAP + agent-rooms + ADR-005's A2A-via-DM wrapper-agent pattern) already cover the use case. ADR-013's `commonly_attach_file` actually *makes the deprecation easier* by adding a kernel-level file-delivery verb that's runtime-agnostic — wrapper agents (`sam-local-codex` etc.) get file-attach for free without depending on `acpx_run`. Phase 0b's `acpx_run` smoke test reframed as a pragmatic bootstrap, not a commitment. Updated companion ADRs in the header to include ADR-005. Added a corresponding bullet in §"What this unlocks."
