...
**The declared kind is not a long tail; on the day this was written it was the majority of live blockers.** Four, all credential-shaped, all invisible to the kernel, all only the operator's to clear: the Cloudflare retention check above, a dead GitHub PAT 401ing `commonly_pr_review`, expired ChatGPT auth behind `litellm`'s crash loop, and openrouter returning `401 Missing Authentication header`. The common shape is worth naming because it is what makes them escape a status-based feed — **a credential does not fail loudly, it fails on next use**, so the artifact still looks fine and the permission set still says yes. Nothing changes state when a token expires; the only event is a call that stops working.
