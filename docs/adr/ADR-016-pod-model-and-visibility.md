...
**Merged is not deployed, and this row is the reason to say so in an ADR rather than only in a checklist (@sprint-review, 2026-08-04).** #797 merged at `07:33:37Z`; the running instance is still `eb05c683` (#793's merge, deployed 2026-08-02) — confirmed by two instruments, the `Deploy Dev` run history and the live `backend` image tag. **So for as long as the dispatch is outstanding, a reader who verifies this section against the live API sees the divergence still open and concludes the pre-#797 text was right.** That is worse than ordinary staleness: a stale claim that a spot-check *contradicts* gets corrected, and one that a spot-check *confirms* hardens. Whoever reads this before the next deploy is being handed the correct answer about `main` and the wrong answer about the system they can actually query — which is the exact reason ADR-017 argues the fact worth routing back to an agent is **merged AND deployed**, two events, not one. The section is written against `main` and it stays true after the dispatch; it is only the window before it that misleads.

**Resolution predicate, because a time-stamped fact in an ADR needs the condition that retires it (@ux-lead).** This paragraph closes when the deployed `backend` image tag is at or past `b2fc6cde` (#797's merge commit); check with `kubectl get deploy backend -n commonly-dev -o jsonpath='{.spec.template.spec.containers[0].image}'`. Until then it is live; after that it is history and should read as history. A dated observation with no retiring condition makes the next reader re-derive it from scratch, which is the work this paragraph exists to save them — and an ADR is the document where that cost compounds longest. **Scale, so the window isn't read as one row waiting on one dispatch:** the last successful `Deploy Dev` was `2026-08-02T02:30:08Z` @ `eb05c683`, and **five** PRs have merged since — #794 `e13bf0fa` (`08-02T03:49:28Z`, ~80 minutes after that deploy), then #796 `2fab7df4`, #797 `b2fc6cde`, #798 `029b8a7c`, #792 `83bf68f9` within nineteen seconds of each other at `08-04T07:33Z`. So the divergence has been open essentially since the last deploy, ~55 hours, and it includes #798 — the fix for the message paging every seat here uses to check the others' claims. Two lags compose: `main` lags what the team knows, and the live instance lags `main`.
