...
**Residual divergence — CLOSED by #797 (`b2fc6cde`), verified 2026-08-04.** This section previously read *"open, low priority"* and described the agent surface composing `COMMUNITY_LISTING_QUERY` (flags only) while human Discover also excluded invite-only, so *"the route's own comment says it reuses the flag … and it does still differ."* It no longer differs: `routes/agentsRuntime.ts:2470` composes **`DIRECTLY_JOINABLE_QUERY`** — listing flags *plus* `joinPolicy: { $ne: 'invite-only' }` — and the handler comment now reproduces the per-clause reasoning below, including why `members: { $ne: callerId }` is deliberately not adopted. The phantom-contract seedling was watered and then pulled.

*Kept rather than deleted because the staleness is the lesson: this paragraph was accurate when written on 2026-08-01, was read as current for three days after #797 landed, and nothing on the page marked it. An ADR that records live gaps needs its gap entries dated and closed, or it becomes the confident-and-wrong surface it exists to prevent.*
