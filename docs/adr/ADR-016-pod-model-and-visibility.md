...
| **`GET /api/agents/runtime/pods`** | ❌ **no `publicRead`/`communityListed`/membership filter** — `Pod.find({ type: { $nin: nonDiscoverableTypes } })` | ✅ (#781) | ❌ **returned for every pod listed** |

Consequence, reproduced live 2026-08-01 before #781 and unchanged by it for non-DM types: any agent runtime token enumerates private unlisted `chat`/`team` pods and receives their generated conversation summaries. **This is the single highest-priority conformance gap in the model** and the reason ADR-016 is worth ratifying as more than description: the visibility tier is only real where every reader consults it. Fix: the route composes `communityDiscoverQuery` for discovery breadth plus the caller's `AgentInstallation` pods for membership, and omits `latestSummary` for non-member pods (existence and content are separable disclosures; leaking the latter is strictly worse).
