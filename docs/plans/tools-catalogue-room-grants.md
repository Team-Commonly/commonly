...
The GitHub tool Installable ships as `source: 'builtin'` with one `McpServer` component (ADR-001 §1), `transport: 'http'`, named for the broker it points at (`commonly-grant-broker`, the MCP server name `routes/mcpGrants.ts` declares). Its `enabledTools` is not hand-written: `backend/scripts/seed-builtin-tools.ts` projects it from the broker's `TOOL_DEFINITIONS` at boot (Wren 67724, on lily-shen's 67719 finding that the catalogue carried no tool row), so the catalogue, the mint and the broker read one list. The mint reads `brokerId` from that seeded component and ignores any `brokerId` in the body; a tool outside `enabledTools` is refused `400 invalid_tools`, and a missing seed row is `503 broker_unavailable`. Named tests: `the builtin GitHub Installable enables exactly the broker's GitHub tools`; `brokerId comes from the tool Installable, never the body`; `the mint refuses a tool the Installable does not enable`. The tiers:
