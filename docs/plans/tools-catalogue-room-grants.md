...
3. **Provider, while the material is still there.** `DELETE /applications/{client_id}/grant` takes the user's access token in its body (Vera 67894), so the provider revoke reads the material one last time and runs **before** anything is revoked locally; it kills the whole authorization, refresh token included. Google is the token-revocation endpoint, same order. A pasted token has no revocation API, so the removal response carries a `revokeAt` URL for the person and the page shows it once.
...
Named tests: `removing a Connection revokes every grant on it`, `the provider revoke runs before the material is gone`, `a failed provider revoke leaves the row and its references for a retry`, `a removed Connection's secret is gone by reference`.
