...
- **Args exist only while the call is pending.** Every terminal transition — approved, declined, expired — atomically `$unset`s `canonicalArgs` and keeps `argsDigest`. A parked comment body does not stay in Mongo after the decision.
