...
The slice needs a credential the server holds so the broker has something to execute with on day one. `backend/services/githubAppService.ts` has the code path to mint short-lived installation access tokens for a GitHub App, server-side, never handed to an agent — but the App itself does not exist yet (verified 2026-09-16, Sam 68709): Team-Commonly has no Commonly GitHub App installed, Secret Manager holds only the login OAuth client and the PAT, and the pod carries `GITHUB_PAT` with no `GITHUB_APP_ID` or private key. The broker never uses the service's PAT mode, so the App must be created and installed first (TASK-033, Sam's; Sam has the App recipe) before the C4 stranger run can reach a real call. Once installed, that is the broker's credential source with no new OAuth app, no verification queue, and no secret material in a row. Gmail needs a Google OAuth client, consent screen review, and refresh-token storage before the first call can happen; it is the right second tool and the wrong first one.
...
| installed per member (before the broker, and after it for a member's own install) | cobalt, solid | **GitHub** · installed by **you** | `your agents may use it · nothing is shared with the room` | `installed {rel}` | Manage (bordered) |
...
### 10.0 The boundary — which rows are on this path, and which never will be

Sam asked (69027, 2026-09-17) whether the connectors already on the page follow this section. They do not, and the reason is a boundary worth writing down before the catalogue grows, because two kinds of row will sit next to each other and look alike. Measured on `main` at `cdaa5879`, not inferred: `Integration.scope` is already `'pod' | 'user'` and user-scoped rows already exist (a Telegram link is a person's own chat; the GitHub App row itself is created at `scope: 'user'`, `routes/integrations.ts`). The grant path is not shared: the mint refuses any row whose `type` is not `github-app` (`routes/grants.ts`, `connection_mismatch`), and every tool in `toolBrokerService.ts` is typed `connectionType: 'github-app'`. So today exactly one row type can be granted to a room or reached through the broker, and no channel can.

**Two questions decide where a provider goes, asked in this order.**

1. **What does the row do?** If it routes a pod's messages in and out — a workspace's bot token, a linked chat id, a webhook the provider calls — it is a **channel connector**: it lives on the Connectors page (`list: 'channels'`, `installableCatalogService.ts`), it is never granted, never attenuated, never brokered, and it never appears in a trail. If it lets an agent act at an outside service on someone's behalf — open an issue, send a mail, send a message to a stranger — it is a **Connection**: it lives on the Tools page (`list: 'tools'`), and everything in pieces 1–5 and this section applies: grant, tiers, attenuation, budget, expiry, confirmation floor, trail.
2. **Whose account does the outside service see?** This question is asked only of Connections. If the provider sees an app or an installation, it is the App row of 10.2 path 2 — an admin installs it and is its granter, and the row names the installation (`owner/repo`), not a person. If the provider sees a **named person**, it is a per-person Connection of 10.1 — the person is its granter, and the row names them.

`scope` answers neither question. A channel row at `scope: 'user'` is still a channel; the App row at `scope: 'user'` is still an installation's credential and not a person's. `scope` says who owns the record and how bare messages route; the two questions above say what the row is *for* and whose it is at the provider. The catalogue's `list` field is the enforcement of question 1, and the mint's `type` check is the enforcement of question 2 — a channel row presented to the mint is refused `connection_mismatch` today and stays refused when a second Connection type is added, which is the test that keeps this boundary: `a channel connector row cannot be granted` (extend `grants.mint.test.js` with a `telegram` row when 10.2 lands; the existing `a tool Installable in the catalogue never renders as a channel row` from piece 2 is the other half).

**Channel connectors do not migrate onto this path.** Slack's token belongs to the workspace, Telegram's link is a chat the bot sits in, Discord's is an application — none is a person's credential at the provider, and modelling one as a per-person Connection would misdescribe it: there would be no `providerLogin` to show, no person whose approval the confirmation floor could ask for, and no one whose leaving a room (10.6) should revoke anything. What belongs on this path is a person's own Gmail, GitHub or, in the one form 2026-09-17's ruling allows (68977–68979), LinkedIn.

**What the page does so the two are not confused.** A per-person Connection row names the person it belongs to (`providerLogin`, and "yours" when the viewer is that person); the App row names the installation and the admin who installed it; a channel row names the workspace or chat and never a person. A row that would have to name a person to be understood is a Connection and belongs on the Tools page; a row that is understood without one is a channel and stays on the Connectors page.
