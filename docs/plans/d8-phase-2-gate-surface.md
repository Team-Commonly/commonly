...
| C | D8 schema: `scope`, optional `podId`, `config.gates`, outbound inversion, PATCH gate writes, migration script, reconciler prune; the admin pause/resume verbs with `paused` in the partial index, `liveClaimStatuses` and `statusForClaim`, the two owner-verb refusals, and the two sweep branches | M–L | — (backend only; the page ignores `gates` until D) |
