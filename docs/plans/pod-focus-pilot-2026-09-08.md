...
Turn-frame budget (Sam 65832): at most 8,000 Unicode code points for the complete rendered focus frame, including delimiters, IDs and metadata. This is a measured character ceiling, not an exact tokenizer claim (UTF-8 ceiling 32,000 bytes). First reserve the full goal/scope, owner identity and label, revision, all selected task IDs and their order. Render task detail labels only into the remaining space, at most 160 characters per title, with an explicit ellipsis and “full task details in board / get_context.” Count the final output and test ten multi-paragraph task titles. Keep full titles in structured reads and board. No truncation of selected IDs or loss of an ordered row. Task assignee/status detail shares the bounded detail allowance; stored Task records are unchanged. If malformed legacy data or an unbounded owner label makes the protected portion itself exceed the ceiling, return `FOCUS_FRAME_PROTECTED_OVERFLOW` and use the finite failure lifecycle below rather than truncate protected fields or spawn a stale/unbounded frame. Do not silently trim away focus under an asset/summary token budget; account for its bounded size explicitly.
...
Deterministic failures have a terminal path (Sprint Review 65843). Use the existing event lifecycle, not indefinite CLI retries: `AgentEventService.list` increments delivery attempts; `garbageCollect` requeues expired delivered events below `AGENT_EVENT_REQUEUE_MAX_ATTEMPTS` and changes them to `failed` when the cap is exhausted. The inspected default is **three total delivery attempts**, with a 10-minute delivered-age threshold and a `*/10` scheduled collection pass; this is not a promise of an exact retry time. Root must verify and record the pilot's effective finite cap and active collection job. No new retry queue, event clone, reset of attempts, or success acknowledgement for failed validation.

On each rejected turn-start frame, emit a bounded structured diagnostic with error code, event ID(s), pod ID, focus revision and measured/allowed size, without copying the oversized content. At cap exhaustion the existing failed-event record is the terminal evidence; the CLI diagnostic supplies the focus-specific cause (the current collector's stored error is a generic cap-exhaustion reason). The existing admin failed-events read exposes the terminal record. Root owns repair: correct the owner/source data through its authorized surface, or explicitly change/clear focus. A repaired revision is read on the next ordinary turn. Failed events remain failed; repairing focus does not silently replay them. Any recovery of old failed work is a separate explicit operator action, not a new replay feature in this pilot.

Test this with controlled time and a deliberately oversized protected owner label: no spawn, no success ack, attempts are not reset; after three deliveries and the collector pass the event is failed and absent from pending reads. Cover all admitted entries in a batch, preserve outcomes for entries already independently handled, and verify a new ordinary event consumes the repaired revision. Separately test a transient read failure that recovers before the cap. These are lifecycle acceptance tests, not a requirement to wait through production retry windows.
...
| Deterministic delivery failure | Oversized protected portion: bounded diagnostic, zero spawns/success acks, three total deliveries then existing collector marks failed; no automatic replay after repair; new ordinary event sees repaired revision | CLI contributor + Sprint Review + root / pending |
