...
| Gap | Evidence | Exit criterion | Lane |
|---|---|---|---|
| B1 Credential exposure through list/read endpoints | #1673's class: a leak masked by a 500 | A **probe-driven audit** of every list/read route that touches Integration, User, AgentCredential, RoomGrant: for stranger / member / owner / admin, assert no secret field in any 2xx body. Vera's two probes generalised into a test matrix in CI. Follow-up: `/api/auth/api-token` is metadata-only; generation is the only raw-token response (design note: `docs/plans/api-token-show-once-2026-09-12.md`). **Live run (2026-09-16, dev `00e7ebc9`):** the 28 matrix routes as anonymous (27× 401, agent-profile 200 with no credential field) and as a seat's own runtime token (24× 401, 2× 404, 2× 200, no credential field or token pattern). Agent-profile's 200 is the intended shape; runtime-integrations returned 0 rows, so that case is **unmeasured**, not passed. **Gap: human-role legs run in CI only until A2 lands.** Running them live would mean creating human accounts on production and handing their tokens to a seat, the credential path A2 removes. The live matrix re-runs the day A2 lands. | Connectors (Vera) |
...
| B3 Rate limits on every DB-touching handler | two CodeQL HIGHs this evening on new handlers | CodeQL stays a required check; a lint or test that new `router.*` handlers carry a limiter. **Landed as the jest guard** `backend/__tests__/unit/routes/routeRateLimitGuard.test.js` (#1682): every `router.<verb>` registration needs a limiter ahead of any auth middleware; pre-existing rows sit in `routeRateLimitGuard.baseline.json`, which may only shrink. **First shrink (landed 2026-09-12):** the 11 `routes/agentsRuntime.ts` rows listed as `limiter-after-auth` — `phase4RateLimit` now runs ahead of `agentRuntimeAuth` / `dualAuth`, their baseline rows are gone, `phase4RateLimit` became a two-tier stack (coarse Cloudflare-keyed IP limiter 3000/60s first, so a rotated Authorization header cannot mint a fresh bucket pre-auth, then the 120/60s per-token tier; `agentsRuntime.rateLimitTiers.test.js` drives the real router), and the scanner closed its three blind spots (file-level `router.use(auth)`, a handler named `*limit*`, `router.route()` chains) | Connectors/Sharpen |
