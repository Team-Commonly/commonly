...
**D3 — A reply to the card is a ruling, through the same verb.** A card's channel record is **durable, not a window** (Vera, 63929): `relayMap` is capped at 100 and a busy chat evicts a card while its row is still `pending`, after which a reply would silently become chat. So the bridge writes a card to `config.openCards: [{ podMessageId, tgMessageId | externalMessageId, sentAt }]` at relay time, beside the ordinary `relayMap` entry, and removes it when the card closes (D6) — never by count. "A card you can still reply to" means *a card in `openCards`*, which is every pending card ever sent to this chat. In both bridges, before the inbound message is posted, a quote-reply is matched against `openCards` first (then `relayMap` for ordinary lines, unchanged), and the hit's `podMessageId` is looked up: `DecisionRequest.findOne({ messageId: podMessageId })`. If it exists and is `pending`, the reply is parsed:
...
**D5 — A bare number resolves the one open card.** People do not quote-reply on a phone. A message that is *only* an integer (`^\s*\d{1,2}\s*$`), sent to a chat that has **exactly one** card in `openCards` whose row is still `pending`, is treated as a reply to that card (D3). Not `DecisionRequest.find({ podId, status: 'pending' })`: under D8 gates one connector relays cards from several pods, and a pending fork that was never sent to this chat is not one the human can see. With two or more pending, the bridge answers `Which one? Reply to the card you mean.` and posts nothing. With none, the number is an ordinary message and relays as today. Anything longer than a bare number never resolves a card without a quote-reply — free-text rulings need the quote so a passing sentence is never read as a ruling.

**D6 — A ruling from the workspace closes the card in the channel.** When `chooseDecision` succeeds, the service asks the bridges to confirm wherever the card was shown: `Integration.find({ 'config.openCards.podMessageId': row.messageId, isActive: true })` → for each, send `✓ Ruled by {by}: {value}` as a reply to that card and `$pull` the `openCards` entry. The reconciler's five-minute sweep does the same pull for any `openCards` entry whose row is **not `pending`** — `ruled`, or missing altogether (a purged decision, a deleted pod) — so a confirmation that failed to send, or a row that vanished, cannot strand an entry (Vera, 63950), and `openCards` cannot grow past the number of open forks. Without this a phone shows an open card for a fork that was ruled at a desk ten minutes earlier, and the next "2" from the phone meets D4 without warning. Fire-and-forget, like every relay; a failed confirmation is a log line, never a failed ruling.
...
| `2` as a quote-reply to a card sent 150 messages ago | that card (`openCards`, not the `relayMap` window) | ruling = option 2's label | `✓ Ruled: {label}` |
...
15. **No window.** Card sent, then 150 ordinary relays into the same chat (the `relayMap` cap is 100) → a quote-reply `2` to the card still rules it, and a bare `2` still counts it as the one open card. After the ruling, the `openCards` entry is gone; after a failed confirmation, the sweep removes it within one pass.
16. **One fork, three phones.** Pod with members A, B, C, all gated on; C has `/mute`d. The card reaches A and B (two `openCards` entries, on two integrations) and not C. A and B reply `2` and `1` concurrently → exactly one ruling; the winner's chat gets `✓ Ruled: …`, the loser's gets `Already ruled … by {winner}: …` and their text is under the ask; C gets nothing at any point, and a bare `2` from C after `/unmute` relays as an ordinary message. Then rule a fresh card from the workspace → A's and B's chats both get `✓ Ruled by {by}: {value}`, C's does not; both `openCards` entries are gone.
...
- **Expiry.** `DecisionRequest` has no expiry and no cancel. A card that is never ruled stays open in both places; when the model gains either, the channel gets a `Withdrawn` line the way D6 gets `Ruled`. There are now two reasons for it, not one: a stale card in two UIs, and `openCards` — bounded by the number of open forks, and read on every inbound message to that chat — which has no other way to shrink for a fork nobody will ever rule (Vera, 63950). Three fields per entry and abandoned cards are rare, so nothing changes today; the next person deciding expiry should see both.
