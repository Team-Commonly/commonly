...
**The ruling.** `chooseDecision({ decisionId, callerUserId, value, origin })` is the only writer (the server supplies `origin`, D6): human pod member only (bots 403), a two-minute `rulingLock` CAS, and the ruling posted as a **threaded human reply to the ask message** (`postRulingReply` → `PGMessage.create(…, replyTo = messageId)`). Once the row is `status: 'ruled'` (and its attention item is resolved), it queues one typed `decision.ruled` event for the asking runtime with the decision id, pick, ruler, timestamp, and ruling-message id. The visible reply deliberately suppresses only the generic implicit-reply wake, so one ruling produces one agent turn while explicit `@` mentions still deliver normally. A second call answers `409 { error: 'Decision already ruled', decision }` with the standing ruling. `value` is free text: the Activity card sends an option's label, or the *Other…* text.
...
**A reply is not an authorization** (Vera, 63917). Before the verb is called, the resolver binds three things, in this order, and stops at the first miss with nothing written: **the chat** — the card is looked up in *this* integration's `cards`, so a reply can only resolve a card that was sent to the chat it came from; **the identity** — the inbound predicates the bridges already refuse on stay in front of the resolver (Telegram `chat.type === 'private'`, the reason group chats are refused; Slack `event.user === config.slackUserId`), and the caller passed to the verb is `config.linkedUserId`, never anything derived from the message; **the card** — `status: 'pending'` at lookup, and the verb's own CAS after it. The call is `chooseDecision({ decisionId, callerUserId: linkedUserId, value, origin: { via: provider, integrationId } })` (both origin fields come from the bridge, D6); the verb re-checks that the linked user is a human pod member **of the card's pod** (`row.podId`), which under D8 gates need not be the chat's active pod. The receive-time membership guard (D8 Phase 2) tests the **active** pod (`integration.podId`) and is reached only when nothing in `cards` matched — the order is card-match → identity → verb, then the ordinary path. So the two guards are not two sentences for one state: a member who has left their active pod but is still in the card's pod can rule from that chat while their bare messages hear *This connector has no active pod…* — same chat, two correct answers, by design, because the card carries its own pod and never depends on which one is active. Seed 12's only channel line is D4's (Vera, 64263 §2.6; ruled 64265). **First reply wins by CAS, not by read-then-write:** the bridge never touches the row — the verb's `findOneAndUpdate({ status: 'pending', rulingLock absent-or-expired })` is the only write, so two replies in the same tick produce exactly one ruling and one `409`. On success the ruling lands in the workspace exactly as an Activity-card ruling does: threaded reply under the ask, asker woken once, attention item resolved, socket broadcast. **The reply is not also posted as an ordinary inbound message** — one human sentence becomes one workspace message, the ruling. The channel confirmation is a reply to the card (`reply_to_message_id` = the card's `tgMessageId`; Slack `thread_ts`), sent by the bridge in its own voice, not attributed to an agent.

**D4 — A late second reply changes nothing, and says so.** A ruled row at lookup, or a `409` carrying a ruled row after a concurrent choice, takes the same late-reply path. The bridge answers in the channel, as a reply to the card: `Already ruled {rel} by {by}: {value}. To change it, the agent asks again — say so in the workspace:` + the thread link. `{rel}` is `just now` below one minute, `{n}m ago` below one hour, `{n}h ago` below one day, and `{n}d ago` thereafter, using whole elapsed units. An absent or invalid `ruling.at` omits the time clause entirely: `Already ruled by {by}: {value}. …` No timestamp is invented. The second reply's text **is** posted into the pod as an ordinary threaded reply under the ask (so the words are not lost and the asker sees them), but the ruling is a recorded fact (ADR-028) and does not move. A reply while the two-minute lock is held by another tab answers `Someone is ruling this right now — try again in a moment.` and posts nothing.

**The verb has these additional outcomes, and a throw must not become an ordinary message** (Vera, 63928). The bridge switches on `status`/`code` and never falls back to "post the text as an ordinary message" — that fallback is exactly what double-posts to the asker in the third case:
...
| `409 ruling_finalize_conflict` — the reply **is** posted, but the row did not flip to `ruled` and no `decision.ruled` event is sent | one threaded reply | `Your ruling reached the workspace, but the card didn't close. Open it in Commonly to confirm:` + link | **nothing** — a second post here is the double-post |
| unclassified throw, including a failure after the verb's durable write | a ruling may already be recorded; its outcome is not confirmed to the bridge | `Couldn't confirm the ruling. Check the thread in Commonly before trying again:` + link | nothing; never retry the verb automatically or claim nothing was saved |
...
**D6 — A ruling closes every channel copy, with the origin preserved.** `chooseDecision` receives an internal `origin: { via: 'telegram' | 'slack' | 'workspace', integrationId? }`. The Activity HTTP route supplies `{ via: 'workspace' }`; each bridge supplies its provider and integration id. The route ignores body-supplied origin, provider and integration identifiers. `integrationId` is an exclusion: accepting it from a browser would let a caller silence someone else's phone. This is provenance supplied by trusted code, not a second authorization mechanism.

The single dispatch site is immediately after the final CAS confirms a non-null `ruled` row, before attention resolution or typed-event delivery can throw, not beside each of the verb's two `200` returns. `backend/services/decisionCardReconcileService.ts` calls `markRuled({ podMessageId: row.messageId, ruledVia: origin.via })` and marks `closedAt` on **every** matching `cards` entry, without filtering by connector liveness, membership, mute, gate, pause or origin. It selects integrations by `config.cards.podMessageId`, then sends `✓ Ruled by {by}: {value}` to eligible copies **except** `origin.integrationId`; the answering bridge supplies its own `✓ Ruled: {value}` line. Each send replies to that integration's own `tgMessageId` / `externalMessageId`. Telegram sends plain text; Slack escapes the ruling text before posting it in the card's thread.

**The closing line rechecks eligibility at ruling time.** The connector must still be live and usable, its linked user must still belong to **the card's pod**, and mute, that pod's gate and admin pause must permit the interrupt. No `linkedUserId` means no line: never substitute `createdBy` for the person receiving it. Leaving the pod or saying *not now* suppresses text; the receipt still closes. Await the local ledger and receipt-write attempts, with failures logged, but detach provider-send completion: a stalled sibling API must not delay the asking agent's typed event, the successful return, or the answering bridge's confirmation. A failed send is logged and never turns a settled ruling into a failed one. Marking rather than pulling keeps D4 reachable even after a sibling's closing line arrives.

**The five-minute sweep repairs receipts, not delivery.** It scans bindings with `cards` regardless of `isActive`, batches decision lookups by distinct `podMessageId`, and prevents overlapping passes in the same process. A failed lookup leaves receipts untouched for a later pass; it is not evidence a row was purged. The case split is:

| decision row | receipt | sweep action |
|---|---|---|
| `pending` | no `closedAt` | leave open |
| `pending` | `closedAt` present | leave unchanged; never unset or prune the mark |
| `ruled` or missing | no `closedAt` | stamp `closedAt` now |
| `ruled` or missing | closed less than seven days ago | leave marked |
| `ruled` or missing | closed at least seven days ago | remove by `podMessageId` and `closedAt` predicate, never by array index |

Pending + `closedAt` is unreachable by construction: PR 2 marks only a ruled row, finalize-conflict does not mark, and this fan-out runs only after settlement. The defensive reply is D3's *This card has closed…* line, not a fabricated standing ruling. The sweep never resends a closing line. After removal, a quote-reply is ordinary chat and rules nothing; until removal, the marked entry keeps the late-reply path available. `cards` is therefore bounded by open forks plus a week of normally closed receipts, with abandoned pending forks still subject to §7's expiry question.
...
| a quote-reply whose decision row is missing | retained receipt | nothing | `This card is no longer available. Open Commonly to check the thread.` |
| a quote-reply with `closedAt` but a pending row | defensive state after membership check | nothing | `This card has closed. Open it in Commonly to confirm:` + link |
| an unclassified throw from the ruling verb | ruling outcome unknown to the bridge | no additional post | `Couldn't confirm the ruling. Check the thread in Commonly before trying again:` + link |
...
| 3 | D6 + D7: origin-aware closing lines, fork-wide `ruledVia`, five-minute receipt sweep and seven-day pruning | S | 2 |
...
9b. **A reply is not an authorization.** (a) A Telegram update whose `chat.type` is `group`, on a chat that somehow matches a live row → dropped before lookup, `chooseDecision` never called. (b) A Slack event with `event.user !== slackUserId` in the DM that received the card → dropped before lookup. (c) A quote-reply in chat B to a `message_id` that exists only in chat A's `cards` → no hit, ordinary message, nothing ruled. (d) The caller recorded on the ruling is `config.linkedUserId`, never the sender name. (e) Two replies to the same card delivered concurrently (two webhook requests in flight) → exactly one `ruling`, one `✓ Ruled`, one `Already ruled` or `Someone is ruling…`; the verb's CAS is the only arbiter and the bridge holds no state of its own.
...
11. **Ledger.** After 3: the answering channel alone has `reachedHumanAt`; every fork row, including held rows, has `ruledVia: 'telegram'` and `expiresAt = now + 90 days`. After 8: every fork row has `ruledVia: 'workspace'` and the renewed expiry; no row gains `reachedHumanAt`.
17. **Internal origin and independent sends.** Forge `origin`, `via` and `integrationId` in the Activity body: the verb still receives workspace origin with no exclusion. A channel ruling excludes only its internally supplied integration. Hold a sibling provider send pending: local receipt closure completes and the verb can deliver its typed event and return without waiting for the send. The same single dispatch remains reached if later attention resolution or typed-event enqueue throws.
18. **Eligibility and sweep.** Test removal from the card's pod, missing `linkedUserId`, mute, gate-off and admin pause independently: each suppresses closing text but preserves unconditional closure. Test every D6 sweep row, including pending + an old `closedAt`, failed lookup, inactive bindings and the seven-day boundary. Exercise both providers and their own reply targets.
...
- **Which surface the asker sees.** The asking runtime receives the typed ruling event; whether the visible reply text says *via Telegram* is the existing `📱 … (via Telegram)` prefix question, unchanged here.
