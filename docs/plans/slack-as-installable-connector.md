...
3. **The callback is the enable command.** `GET /api/webhooks/slack/oauth/callback?code&state`
   is unauthenticated, exactly as the Telegram webhook is. It resolves the row by
   `{ type: 'slack', isActive: true, 'config.connectCode': state }`, refuses expired or unknown
   states (single-use, same `isConnectCodeExpired` fail-closed), exchanges `code` at
   `oauth.v2.access`, and only on success binds:
   `config.teamId`, `config.slackUserId` (`authed_user.id`), `config.chatId` = the IM channel from
   `conversations.open({ users: authed_user.id })`, `config.chatType: 'im'`, `status: 'connected'`,
   code fields cleared. The installer's identity was stamped at projection (`linkedUserId`), so the
   callback stamps nothing — it proves the Slack side. A failed exchange writes nothing and shows the
   page's "expired or invalid" copy; *New code* re-mints.
...
consent → callback binds team, user, DM and stores the token behind the ref → card flips to
...
limiter (it mints nothing but it binds); `state` is verified before `oauth.v2.access` is called, so an
attacker cannot spend our exchange budget with forged states; the exchange response is never logged;
...
3. Callback with a valid `state` → exchange once, `ConnectorSecret` written, row bound
   (`teamId`, `slackUserId`, `chatId`, `chatType: 'im'`, `status: 'connected'`), code cleared,
   `botTokenRef` present and `botToken` absent from the row and from every API response.
...
| `authorize-url` route, OAuth callback route, events + commands routes with signature/ack/dedupe | Kai | M |
