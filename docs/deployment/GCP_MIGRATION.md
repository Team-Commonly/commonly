...
This document covers how to migrate the Commonly Kubernetes cluster from one GCP project/account to another. Performed 2026-03-04: `commonly-test` (YOUR_CODEX_ACCOUNT_3) → `YOUR_OLD_GCP_PROJECT_ID` (YOUR_CODEX_ACCOUNT_2).
...
| Field | Value |
|-------|-------|
| GCP Account | `YOUR_CODEX_ACCOUNT_2` |
| Project ID | `YOUR_OLD_GCP_PROJECT_ID` |
...
| Image registry | `gcr.io/YOUR_OLD_GCP_PROJECT_ID/` |
| Skills catalog GCS | `gs://YOUR_OLD_GCP_PROJECT_ID_cloudbuild/awesome-agent-skills-index.json` |
...
```bash
NEW_PROJECT=YOUR_OLD_GCP_PROJECT_ID
...
The Cloudflare tunnel (`YOUR_CLOUDFLARE_TUNNEL_ID`) is account-scoped to Cloudflare, not to GCP. It does **not** need to be recreated when migrating GCP projects. The tunnel credentials live in the `cloudflared-commonly-k8s` secret in `ingress-nginx`. As long as that secret is restored, the tunnel connects automatically.
