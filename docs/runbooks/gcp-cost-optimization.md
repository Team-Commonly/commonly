...
| dev-pool (1× n2-standard-2 on-demand, autoscale 1-3) | Agent runtimes — `cloud-codex-*`, `commonly-bot`, and `litellm`. Taint `pool=dev:NoSchedule`. | $2.33 |
...
`spot-pool`. Saved ~$50/mo. Agent runtimes (`cloud-codex-*`,
`commonly-bot`, and `litellm`) stay on `dev-pool` because spot preemption
