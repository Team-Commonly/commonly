...
| — | **an unknown flag is refused (exit 2), not ignored — and so is a value that belongs to no flag.** That is the defect this table was written for: the script had no `--width` at all, so `--width 390 --out x-390.png` wrote a silent 1440×900 capture under a `-390` name and the reviewer had no way to tell (2026-09-19). The second half is the same class one step over: `--width 390 400` used to run with 390 and never mention the 400. Check the run line: `viewport` and `auth` are printed on every capture for exactly this reason. |
