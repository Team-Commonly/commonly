...
export const buildTools = (config) => {
  ...
  const tools = [
    ...
    },
    {
      ...
      description: 'Create a task in the pod task board. `dep` is a blocking dependency taskId; `parentTask` is hierarchical. `sourceRef` + `title` are an idempotency key: re-sending the same pair returns the existing task instead of creating a second one, while the same `sourceRef` with a different `title` is a different ask and gets its own task.',
      inputSchema: reqWith({
        podId: STRING,
        title: STRING,
        assignee: STRING,
