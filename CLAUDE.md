...
- **Latest backend image**: `gcr.io/disco-catcher-490606-b0/commonly-backend:20260329003026`
- **Latest gateway image**: `gcr.io/disco-catcher-490606-b0/clawdbot-gateway:20260329001109`
...
22. **acpx_run always routes through LiteLLM; OpenRouter 401 fix** (backend `20260329003026`, gateway `20260329001109`, helm rev 111) — Root cause of "Codex rate-limited on all configured accounts" despite available quota: `readAgentLiteLLMKey` read `openrouter:default.key` but dev agents have the LiteLLM key in `openai-codex:codex-cli.access` → LiteLLM path never entered → fell to direct OAuth (accounts 1&2 share same rate-limit pool, both exhausted). Fixes: (a) `tools.ts`: `readAgentLiteLLMKey` now reads `openai-codex:codex-cli.access` (primary) with legacy fallback to non-sk-or-v1- key at `openrouter:default.key`; (b) `agentProvisionerServiceK8s.js`: `injectOpenRouterKeyToAgentAuthProfiles` writes real `OPENROUTER_API_KEY` to `openrouter:default.{key,apiKey}` so OpenClaw heartbeat OpenRouter fallbacks work; LiteLLM virtual key goes only to `codex-cli.access`; (c) removed `//` comment from join-space exec script — it commented out `store.order`, `fs.writeFileSync`, and `catch` block (→ SyntaxError, exit code 1) since the codexOrder fix was added. Key layout now: `codex-cli.access=sk-xxx` (LiteLLM), `openrouter:default.key=sk-or-v1-xxx` (real OR key).
...
- **Three Codex accounts (as of 2026-03-24)**: account-1 (`openai-codex:codex-cli` / `openai-codex:default`), account-2 (`openai-codex:account-2`), account-3 (`openai-codex:account-3`). Rotation order for dev agents: `["openai-codex:codex-cli","openai-codex:account-2","openai-codex:account-3"]`. Account-3 is a **different ChatGPT account** and has its own separate rate limit pool — rotation to it DOES help when accounts 1 and 2 are exhausted.
- **Both Codex accounts-1 and -2 share the same rate limit**: `openai-codex:codex-cli` and `openai-codex:account-2` both have `chatgpt_account_id: 66acfb97-6030-4a58-9d4e-135eadca109d` — same ChatGPT team plan, shared rate limit pool. Account-3 is on a separate plan.
