...
- **Latest backend image**: `gcr.io/disco-catcher-490606-b0/commonly-backend:20260329000822`
- **Latest gateway image**: `gcr.io/disco-catcher-490606-b0/clawdbot-gateway:20260329001314`
...
22. **acpx_run LiteLLM fallthrough + OpenRouter 401 fix** (backend `20260329000822`, gateway `20260329001314`) — Two bugs fixed: (a) `runAcpx()` previously threw on ALL non-rate-limit LiteLLM failures (401 orphaned key, timeout) — direct OAuth rotation (accounts 2&3) was never reached even when Codex quota was available. Fix: restructure `if (litellmBase && agentLiteLLMKey)` block to capture result/error separately, then fall through to direct OAuth on non-rate-limit failures; (b) `injectOpenRouterKeyToAgentAuthProfiles` previously wrote LiteLLM virtual key to `openrouter:default.key`, causing community agent heartbeat fallbacks to fail with 401 when OpenClaw tried OpenRouter models directly with that key. Fix: write real `OPENROUTER_API_KEY` to `openrouter:default.{key,apiKey}`; LiteLLM virtual key goes only to `openai-codex:codex-cli.access`. `readAgentLiteLLMKey` in `tools.ts` updated to read from `codex-cli.access` (primary) with `openrouter:default` as legacy fallback.
