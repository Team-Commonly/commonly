...
- **Verify a ship at the CONSUMER, not at the registry or the workflow.** Two different indirections bit this on 2026-08-19. (a) `npm publish` does not reach the local fleet: mcp@0.3.2 was published, unpacked and content-verified, and still reached none of the five working seats. **The two locators this entry originally gave for that are both wrong, and re-measuring them on 2026-08-30 found three copies at three versions.** The MCP path is NOT in `~/.commonly/tokens/<agent>.json` — checked all 20+ of them, none carries an mcp path in any value; the keys are `adapter/agentName/environment/instanceId/instanceUrl/podId/runtimeToken/savedAt/workspacePath`. The path a seat actually loads is in its **spawn argv**: `--mcp-config <tmpdir>/mcp-config.json`, whose `mcpServers.commonly.args[0]` named `~/.commonly/mcp-staging/commonly-mcp/src/index.js` — a hand-patched copy at **0.3.4**, carrying `package.json.bak-0.3.0` and `.bak-0.3.1` beside the live one. That tmpdir is regenerated per spawn, so **argv at runtime is the only instrument that answers 'which build is this seat running'** — there is no durable file to grep. Meanwhile `/opt/homebrew/bin/commonly` is now an ordinary global install (`../lib/node_modules/@commonlyai/cli/src/index.js`, cli **0.1.24**), not a worktree; the worktree symlink migrated to the *mcp* package (`/opt/homebrew/lib/node_modules/@commonlyai/mcp` → a `~/.claude/jobs/…/tmp/` checkout, **0.3.0**), which the seat does not load. Three consumers, three versions, none of them main. **A locator decays faster than the rule it supports** — the rule below held for a year while both of its paths moved underneath it, so re-derive the path before trusting it to check the version. (b) A deploy's green tick is not the enforcement boundary — Kubernetes serves from the OLD pod through a rolling update, so take the cutover from `kubectl get pod -o jsonpath='{.status.startTime}'`. Splitting a measurement on the workflow's completion time put a pre-fix run inside the "enforcing" window and made a working change look broken. The existing "smoke the shipped artifact" rule is necessary and insufficient: it proves the artifact is correct, never that the consumer loads it. Full write-ups: AX audit entries 34 and 35.
